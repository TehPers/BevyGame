...
pub enum BodyType {
    ...
    /// The physics engine will ignore this body. It will not be moved by the
    /// physics engine, and collisions will not be calculated with this body
    /// and any other body. The physics engine will still update the body's
    /// `Transform` component, so other systems should set its position via the
    /// `AxisAlignedBoundingBox` component still.
    Disabled,
}
...
#[derive(Clone, Copy, PartialEq, Debug, Display, From, Into)]
